<form method="POST" action="{{route('agencies.update', ['agency' => $agency->id])}}" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div class="form-group row">
        ...
        <div class="col-md-6">
            <input type="text" class="form-control @error('name_en') is-invalid @enderror" name="name_en"
                   value="{{old('name_en') ?? $agency->name_en}}" id="name_en" required>
        </div>
    </div>
    ...
    <div class="form-group row">
        ...
        <div class="col-md-6">
            <textarea name="description" class="form-control @error('description') is-invalid @enderror" required
                      rows="4" cols="50" id="description">{{old('description') ?? $agency->description}}</textarea>
        </div>
    </div>
    ...
    <div class="form-group row">
        ...
        <div class="col-md-6">
            <input type="text" class="form-control @error('hotline') is-invalid @enderror" name="hotline" id="hotline"
                   value="{{old('hotline') ?? $agency->hotline}}">
        </div>
    </div>
